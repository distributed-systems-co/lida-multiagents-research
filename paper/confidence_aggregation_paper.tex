...
\title{\textbf{A Comprehensive Framework for Multi-Source Confidence Aggregation: 169 Strategies Across 20 Methodological Categories}}
...
\begin{abstract}
We present a comprehensive framework for aggregating confidence scores from multiple heterogeneous sources, implementing and evaluating 169 distinct aggregation strategies spanning 20 methodological categories. Our framework addresses a fundamental challenge in ensemble systems, multi-agent deliberation, and distributed AI: how to optimally combine probabilistic assessments from sources with varying reliability, expertise, and potential biases. We introduce novel strategies from information geometry (Fisher-Rao geodesics, Bregman centroids, $\alpha$-divergence minimization), optimal transport (Wasserstein barycenters, Sinkhorn divergence, sliced Wasserstein), quantum-inspired computing (superposition, entanglement, annealing), and game theory (Shapley values, Nash bargaining, nucleolus). Through extensive empirical evaluation across 5,070 experimental conditions (169 strategies $\times$ 10 scenarios $\times$ 3 iterations), we demonstrate that no single strategy dominates across all scenarios, but identify Pareto-optimal strategies achieving superior speed-robustness tradeoffs. Three strategies achieve perfect outlier immunity: HISTOGRAM\_DENSITY, UCB\_AGGREGATION, and MAJORITY\_VOTE. Execution times span three orders of magnitude (0.02ms to 20ms), with sampling methods being slowest due to Monte Carlo iterations. Our intelligent meta-aggregator automatically selects appropriate strategies based on detected data characteristics (high agreement, outliers, bimodality, skewness), achieving robust performance across diverse conditions. The complete implementation is provided as an open-source Python library integrated with the LIDA multi-agent research platform.

\vspace{0.5em}
\noindent\textbf{Keywords:} confidence aggregation, ensemble methods, belief combination, robust estimation, Bayesian inference, multi-agent systems, uncertainty quantification, Dempster-Shafer theory, optimal transport, information geometry
\end{abstract}
\vspace{1em}
...
The problem of combining probabilistic assessments from multiple sources---variously termed \emph{opinion pooling}, \emph{belief aggregation}, \emph{probability combination}, or \emph{forecast fusion}---is among the oldest problems in probability theory and statistics, dating to the work of Condorcet (1785) on jury voting and Laplace's analysis of witness testimony reliability.

In modern machine learning and artificial intelligence, this problem has gained renewed urgency. Large language models (LLMs) exhibit variable reliability across domains, necessitating ensemble approaches. Multi-agent systems require principled mechanisms for collective decision-making. Sensor networks must fuse readings from devices with different noise characteristics. Crowdsourcing platforms aggregate annotations from workers of varying expertise.

\subsection{Motivating Example}

Consider the scenario of aggregating confidence scores from multiple AI models evaluating a claim:
...
What should be the aggregate confidence? Different methods yield different answers:

\begin{itemize}[noitemsep,topsep=0pt]
    \item Weighted average: 0.7873
    \item Median: 0.78
    \item Log-odds pooling: 0.8039
    \item Robust Huber: 0.8124
    \item Dempster-Shafer: 0.9998
\end{itemize}

The 27\% range between methods (0.78 to 0.9998) demonstrates that aggregation method choice profoundly impacts results. This motivates our comprehensive evaluation of 169 methods.

\subsection{Historical Context}

The mathematical study of opinion combination traces through several intellectual traditions:

\paragraph{Probability Theory (18th-19th Century).} Condorcet's jury theorem (1785) showed that majority voting among independent jurors with better-than-random accuracy converges to truth. Laplace extended this to weighted combinations based on witness reliability.

\paragraph{Statistics (20th Century).} Fisher's method for combining p-values (1925), Stouffer's z-score method (1949), and meta-analysis frameworks formalized statistical approaches. Robust statistics (Huber 1964, Tukey 1977) addressed sensitivity to outliers.

\paragraph{Belief Functions (1970s-1980s).} Dempster-Shafer theory (Dempster 1967, Shafer 1976) extended probability to handle uncertainty and ignorance. Subjective logic (Jøsang 1997) added explicit uncertainty modeling.

\paragraph{Machine Learning (1990s-Present).} Ensemble methods (bagging, boosting, stacking) demonstrated that combining weak learners produces strong learners. Neural attention mechanisms (2017) enabled learned aggregation.

\subsection{The Aggregation Zoo}

The literature contains hundreds of aggregation methods, scattered across statistics, decision theory, AI, and domain-specific applications. No unified treatment exists. We organize 169 methods into 20 categories:

\begin{enumerate}[noitemsep,topsep=2pt]
    \item Basic (11): means, medians, votes
    \item Bayesian (10): log-odds, hierarchical, empirical
    \item Robust (10): Huber, Tukey, breakdown-point
    \item Density (14): KDE, GMM, Dirichlet process
    \item Sampling (12): bootstrap, MCMC, SMC
    \item Information (12): entropy, KL, mutual information
    \item Belief (10): Dempster-Shafer, subjective logic
    \item Transport (5): Wasserstein, Sinkhorn
    \item Spectral (5): Laplacian, diffusion maps
    \item Geometry (5): Fisher-Rao, Bregman
    \item Neural (8): attention, transformers, deep sets
    \item Probabilistic (7): EP, VMP, VI
    \item Hybrid (17): multi-scale, graph-based
    \item Game (5): Shapley, Nash, nucleolus
    \item Causal (5): do-calculus, IV, DML
    \item Conformal (4): prediction intervals
    \item Meta (8): stacking, super learner, UCB
    \item Quantum (4): superposition, annealing
    \item Prompt (8): LLM-based density
    \item Other (9): stable distributions, etc.
\end{enumerate}

\subsection{Contributions}

\begin{enumerate}[noitemsep,topsep=2pt]
    \item \textbf{Comprehensive Taxonomy}: 169 strategies in 20 categories with unified API
    \item \textbf{Novel Methods}: First application of information geometry and optimal transport to confidence aggregation
    \item \textbf{Rigorous Evaluation}: 5,070 experiments measuring speed, robustness, efficiency
    \item \textbf{Intelligent Selection}: Auto-detection of data characteristics for strategy recommendation
    \item \textbf{Production System}: Open-source implementation integrated with LIDA platform
\end{enumerate}
...
The problem of aggregating probabilistic assessments has a rich history spanning multiple disciplines. We organize related work into seven major streams.
...
\textbf{Quantum (4)}: Superposition, entanglement, annealing, amplitude estimation.
...
\begin{itemize}[noitemsep]
    \item \textbf{Speed}: Mean execution time (ms)
    \item \textbf{Outlier Sensitivity}: $|\hat{c}_{\text{with}} - \hat{c}_{\text{without}}|$
    \item \textbf{Noise Sensitivity}: Change under perturbation
    \item \textbf{Sample Efficiency}: Performance with few sources
\end{itemize}

\subsection{Hardware}

Apple M-series, 16GB RAM, Python 3.11.
...
\begin{itemize}[noitemsep]
    \item Synthetic benchmarks may miss real complexity
    \item Fixed hyperparameters
    \item Independence assumption (no correlation modeling)
    \item Single-run timing variability
\end{itemize}
...
The sample median has breakdown point $\varepsilon^* = 0.5$.
\end{theorem}

\begin{proof}
\textbf{Upper bound:} Replacing $\lfloor n/2 \rfloor + 1$ observations with $+\infty$ makes the median $+\infty$. Thus $\varepsilon^* \leq 0.5$.

\textbf{Lower bound:} With fewer than $\lfloor n/2 \rfloor$ outliers, at least $\lceil n/2 \rceil$ original observations remain. The median lies within the range of these, hence bounded. Thus $\varepsilon^* \geq 0.5$.
\end{proof}
